...
type StreamTests() = 
    ...
    member x.TestWriteTo() =
        ...
        event "StreamTests.TestWriteTo-0" |> writeTo Stream.Empty |> ignore
    ...
    member x.TestWriteToShouldFailWithIndexTooLow() =
        let writeTo, readFrom =
            let connection = connect()
            writerTo connection, readerFrom connection
        event "StreamTests.TestWriteToShouldFailWithIndexTooLow-0" |> writeTo Stream.Empty |> ignore
    ...
    member x.TestWriteToShouldFailWithIndexTooHigh() =
        let writeTo, readFrom =
            let connection = connect()
            writerTo connection, readerFrom connection
        event "StreamTests.TestWriteToShouldFailWithIndexTooHigh-0" |> writeTo Stream.Empty |> ignore
