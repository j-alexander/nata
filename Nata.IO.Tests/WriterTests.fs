...
type WriterTests() =
    ...
    member x.MapMetadataValueTest() =
        let read, write = x.Connect(Writer.mapMetadata (fun i -> i*i))
    ...
    member x.MapMetadataTypeTest() =
        let read, write = x.Connect(Writer.mapMetadata int64)
    ...
    [<Test>]
    member x.MapTest() =
        let read, write = x.Connect(Writer.map (fun (x:int) -> int64 (x*x)) (fun (x:int) -> (1+x).ToString()))
