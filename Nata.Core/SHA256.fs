...
let processChunk (h0,h1,h2,h3,h4,h5,h6,h7) (chunk:uint32[]) =
    ...
    let (a,b,c,d,e,f,g,h) =
        [0..63]
        |> List.map (fun i -> k.[i], w.[i])
        |> List.fold compress (h0,h1,h2,h3,h4,h5,h6,h7)
...
let reference (data:byte[]) =
    (new System.Security.Cryptography.SHA256Managed()).ComputeHash(data)
    |> Seq.map (sprintf "%02x")
    |> String.Concat
...
let testCompress() =
    ...
    let h0,h1,h2,h3,h4,h5,h6,h7 =
        compress (a,b,c,d,e,f,g,h) (k,w)

    assert(0xe620b22bu = h0)
    assert(0x87564c0cu = h1)
    assert(0xf1369725u = h2)
    assert(0x82e6d493u = h3)
    assert(0xadcef783u = h4)
    assert(0xdd9eff54u = h5)
    assert(0xe07c2655u = h6)
    assert(0xa41f32e7u = h7)

let toHexes (h0,h1,h2,h3,h4,h5,h6,h7) =
    let h:uint32[] = [| h0; h1; h2; h3; h4; h5; h6; h7 |]
    h
    |> Seq.map BitConverter.GetBytes
    |> Seq.map (Seq.rev >> Seq.map (sprintf "%02x") >> String.Concat)
    |> Seq.toArray