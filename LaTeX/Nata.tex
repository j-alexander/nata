...
Parse text when it’s acceptable to fail with \mintinline{fsharp}{None}. Encode as JSON using an ISO 8601 round-trip string.
\begin{minted}{fsharp}
    open FSharp.Data
    open Nata.Core.JsonValue
...
\end{minted}
\emph{Semantics:} each function zeroes lower-order components (e.g., \mintinline{fsharp}{minute} keeps year/month/day/hour and sets minute’s lower parts to zero). These are useful for binning, windowing, and summarization.
...
\begin{itemize}
  ...
  \item A JsonValue-based API for selecting values inside a parsed FSharp.Data \mintinline{fsharp}{JsonValue} (excellent for in-memory traversals and slicing).
\end{itemize}

In other words: if you have a sequence of JSON strings, use the string-based helpers; if you have a parsed \mintinline{fsharp}{JsonValue}, use the JsonValue helpers.
...
\hline
\mintinline{fsharp}{.name} & Exact property descent \\
\mintinline{fsharp}{..name} & Recursive descent (match here and deeper) \\
\mintinline{fsharp}{"*"} & A wildcard property name \\
\mintinline{fsharp}{[*]} & All array elements \\
\mintinline{fsharp}{[0,2,-1]} & Literal indices (negatives count from the end) \\
\mintinline{fsharp}{[start:finish:step]} & Slice; start/finish optional; step defaults to 1 \\
\mintinline{fsharp}{..lines[*].sku} & Combine segments to traverse structure \\
\end{tabular}
\end{center}
...
At the core is \mintinline{fsharp}{strings}:
...
\begin{itemize}
  \item Pull the first available value: \mintinline{fsharp}{Query.first "\$.payload.correlationId" json}
  \item Extract every occurrence across nested arrays: \mintinline{fsharp}{Query.find "\$..metrics[*].name" stream}
  \item Build CSV lines on the fly for archival or quick inspection: \mintinline{fsharp}{Query.findFieldsCsv [ "\$.t"; "\$.id"; "\$.value" ] stream}
\end{itemize}

\subsubsection{Selecting from JsonValue with structural traversal}

Sometimes your pipeline already holds parsed \mintinline{fsharp}{JsonValue} values—perhaps due to upstream validation, schema checks, or prior projections. Using the syntax summarized above, the JsonValue module provides a structural, automata-driven traversal well-suited to recursive descent and array slicing.
...
\begin{itemize}
  \item Last element: \mintinline{fsharp}{[-1]}
  \item First N elements: \mintinline{fsharp}{[0:N]}
  \item Every other element: \mintinline{fsharp}{[::2]}
  \item A window: \mintinline{fsharp}{[start:finish:step]}
\end{itemize}

\paragraph{Typical uses}
\begin{itemize}
  \item Fetch the last line item per order: \mintinline{fsharp}{JsonValue.findSeq ".orders[*].lines[-1]" json}
  \item Recursively pick all occurrences of a property: \mintinline{fsharp}{JsonValue.findSeq "..correlationId" json}
  \item Slice efficiently without materializing entire arrays: \mintinline{fsharp}{JsonValue.findSeq ".frames[100:200:5]" json}
\end{itemize}

\subsubsection{Choosing between the two}

\begin{itemize}
  \item You have a stream of raw JSON strings: favor \mintinline{fsharp}{Query.find} and friends (\mintinline{fsharp}{strings}, \mintinline{fsharp}{first}, \mintinline{fsharp}{fields}, \mintinline{fsharp}{findFields}, \mintinline{fsharp}{findFieldsCsv}). These compose naturally with \mintinline{fsharp}{Seq} and work well for quick projections and exports.
  \item You already have \mintinline{fsharp}{JsonValue}: favor \mintinline{fsharp}{JsonValue.findSeq/find/findList/tryFind}, especially when you want recursive descent or array slicing/negative indexing behavior.
\end{itemize}
